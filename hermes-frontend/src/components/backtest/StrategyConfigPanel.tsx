...
const STRATEGY_CONFIGS: StrategyParamsMap = {
    RSIStrategy: [
        { key: "period", label: "RSI Period", type: "number", defaultValue: 14, min: 2, max: 50, step: 1, description: "Lookback period for RSI calculation." },
        { key: "overbought", label: "Overbought Level", type: "number", defaultValue: 70, min: 50, max: 95, step: 1, description: "Level above which asset is considered overbought (Sell Signal)." },
        { key: "oversold", label: "Oversold Level", type: "number", defaultValue: 30, min: 5, max: 50, step: 1, description: "Level below which asset is considered oversold (Buy Signal)." }
    ],
    SMAStrategy: [
        { key: "fast_window", label: "Fast MA Period", type: "number", defaultValue: 10, min: 2, max: 100, step: 1 },
        { key: "slow_window", label: "Slow MA Period", type: "number", defaultValue: 50, min: 5, max: 200, step: 1 }
    ],
    BollingerBandsStrategy: [
        { key: "window", label: "Period", type: "number", defaultValue: 20, min: 5, max: 50, step: 1 },
        { key: "num_std", label: "Std Dev Multiplier", type: "number", defaultValue: 2, min: 0.5, max: 4, step: 0.1 }
    ],
    MACDStrategy: [
        { key: "fast_period", label: "Fast Period", type: "number", defaultValue: 12, min: 2, max: 50 },
        { key: "slow_period", label: "Slow Period", type: "number", defaultValue: 26, min: 5, max: 100 },
        { key: "signal_period", label: "Signal Period", type: "number", defaultValue: 9, min: 2, max: 30 }
    ],
    MTFTrendFollowingStrategy: [
        { key: "fast_ema", label: "Fast EMA", type: "number", defaultValue: 50, min: 10, max: 200 },
        { key: "slow_ema", label: "Slow EMA", type: "number", defaultValue: 200, min: 50, max: 400 },
        { key: "risk_percent", label: "Risk Percent", type: "number", defaultValue: 0.02, min: 0.01, max: 0.1, step: 0.01 }
    ]
};
...
export function StrategyConfigPanel({ strategyName, onParamsChange, currentParams }: StrategyConfigPanelProps) {
    const config = useMemo(() => STRATEGY_CONFIGS[strategyName] || [], [strategyName]);
    ...
    useEffect(() => {
        ...
        if (needsUpdate || Object.keys(currentParams).length === 0) {
            ...
            config.forEach(p => {
                finalParams[p.key] = currentParams[p.key] !== undefined ? currentParams[p.key] : p.defaultValue;
            });
                    ...
                    {config.map((param) => (
                        <div key={param.key} className="space-y-3">
                            <div className="flex justify-between items-center">
                                <Label htmlFor={param.key} className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
                                    {param.label}
                                </Label>
                            ...
                            {param.type === 'boolean' && (
                                <div className="flex items-center justify-between">
                                    <span className="text-sm text-muted-foreground">{param.description || param.label}</span>
                                    <Switch
